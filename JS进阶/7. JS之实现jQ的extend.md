...
```
在实现上，核心的部分还是跟上篇实现的深浅拷贝函数一致，如果要复制的对象的属性值是一个对象，就递归调用 extend。不过 extend 的实现中，多了很多细节上的判断，
比如第一个参数是否是布尔值，target 是否是一个对象，不传参数时的默认值等。
...
所以在这里我们还要判断是不是函数，这时候我们便可以使用《JS之类型判断(上)》中写得 isFunction 函数
...
结合着《JS之类型判断(下)》中的 isPlainObject 函数，我们可以对类型进行更细致的划分：
...
![image](https://user-images.githubusercontent.com/19302489/149298842-cf3fa1b8-2022-4603-b395-f86e85d85b1a.png)
